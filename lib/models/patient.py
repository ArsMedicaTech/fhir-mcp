...
from lib.models.common import FHIRResource, Reference, Coding, Contact
...
class Patient(FHIRResource):
    ...
    @property
    def address(self) -> List[Dict]:
        return self.data.get("address", [])
    
    @property
    def managing_organization(self) -> Reference:
        org_data = self.data.get("managingOrganization", {})
        return Reference(
            reference=org_data.get("reference", ""),
            display=org_data.get("display", "")
        )
    
    @property
    def contact(self) -> List[Contact]:
        contacts_data = self.data.get("contact", [])
        return [Contact(contact) for contact in contacts_data]
    
    @property
    def communication(self) -> List[Dict]:
